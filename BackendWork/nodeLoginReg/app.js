...
if(req.method=="OPTIONS"){
 ...
 if(req.url=='/login' && req.method=="POST"){
    ...
    req.on('end',async()=>{
        try{
            console.log(body);
            const {email,password}=JSON.parse(body);//convert into jSON object
            const data=await fs.readFile('student.json',{encoding:'utf-8'});
            arr=JSON.parse(data);
            const status=arr.find((ele)=>ele.email==email & ele.password==password);
            if(status){
                return res.end(JSON.stringify({"message":"Successfully login"}));
            }
            else{
...
server.listen(PORT,()=>{
    ...
    console.log("Server is the running on:"+PORT);
})